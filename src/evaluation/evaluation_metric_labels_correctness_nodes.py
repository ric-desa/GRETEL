from src.evaluation.evaluation_metric_base import EvaluationMetric
from src.core.oracle_base import Oracle
from src.core.explainer_base import Explainer




class LabelsCorrectnessNodesMetric(EvaluationMetric):
    """Verifies that the class of the nodes of the counterfactual example are different from that of the original instance
    """

    def __init__(self, config_dict=None) -> None:
        super().__init__(config_dict)
        self._name = 'Node Label Correctness'

    def evaluate(self, instance_1 , instances_2 , oracle : Oracle=None, explainer : Explainer=None, dataset = None):
        r=[]
        for node_id, instance_2 in enumerate(instances_2):
            label_instance_1 = oracle.predict(instance_1)[node_id]
            label_instance_2 = oracle.predict(instance_2)[node_id]
            oracle._call_counter -= 2

            result = (int(label_instance_1), int(label_instance_2))
            
            r.append(result)
        
        # print(r); input()
            
        return r